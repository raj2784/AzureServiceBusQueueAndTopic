...
namespace SenderQueue
{
    ...
    class SenderQueueClass
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            while (true)
            {
                Console.WriteLine("This is Sender Queue");
                Console.WriteLine("Plaease Enter Message (exit to terminate)");
                string newMessage = Console.ReadLine();
                if (newMessage == "exit")
                    break;
                var msg = new ServiceBusMessage(newMessage);

                // Messsage properties can be added as per below, as per application requeiremenet
